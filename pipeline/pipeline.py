...
from src.LLM_run import run_LLM, available_llms
from src.binary_filter import run_filter
from src.inf_rebel import test_model as rebel
from src.st1_combine import main_st1
from src.st2_combine import main_st2
...
def run_pipeline(config):
    ...
    if not user_flag and os.path.exists(args.st0_preset) and args.override_preset == 'False':
        ...
        if user_flag == False:
            df_final.to_csv(
                args.preset_cache_dir + 'tf-' + args.test_file[9:] + '-filter-roberta-' + st0_path[-1] + '.csv')
    ...
    if args.subtask3_flag == 'True':
        ...
        if args.rebel_flag == 'True':
            ...
            if user_flag:
                if same_model:
                    if st_switch == 'st1':
                        rebel_df = rebel(args.only_causal, args.rebel_st1_mod)
                    if st_switch == 'st2':
                        rebel_df = rebel(args.only_causal, args.rebel_st2_mod)
                    else:
                        rebel_df = rebel(args.only_causal, args.rebel_st2_mod)
                ...
                if 'rebel' in args.st1_preset and os.path.exists(args.st1_preset) and args.override_preset == 'False':
                    ...
                    rebel_st1_preset_name = {
                        'label': st0_preset_name + '-st1-rebel-' + rebel_path[-1]
                    }
                    rebel_st2_preset_name = {
                        'span_pred': st0_preset_name + '-st2-rebel-' + rebel_path[-1]
                    }
